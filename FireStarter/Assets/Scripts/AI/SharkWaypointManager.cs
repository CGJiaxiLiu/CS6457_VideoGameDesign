...
public class SharkWaypointManager : MonoBehaviour
{
    public List<Transform> Waypoints = new List<Transform>();
    public List<GameObject> DeadFish = new List<GameObject>();
    /*
    ...
    */
    ...
    {
        DeadFish.Clear();
        GameObject[] dflist = GameObject.FindGameObjectsWithTag("Dead Fish");
        foreach(GameObject df in dflist)
        {
            DeadFish.Add(df);
        }
    }
    ...
    void SpawnNPC()
    {
        foreach(AIObjects AIO in AIObject)
        {
            if (AIO.enableSpawner && AIO.objectPrefab != null)
            {
                ...
                {
                    for (int y = 0; y < Random.Range(0,AIO.spawnAmount); y++)
                    {
                        ...
                        float tempy = tempSpawn.transform.position.y;
    ...
    */
    ...
    public GameObject ClosestDeadFish(Vector3 SharkPos)
    {
        float distance = 0f;
        GameObject closestDeadFish = null;
        foreach (GameObject DF  in DeadFish)
        {
            if (DF != null)
            {
                float currDist = Vector3.Distance(DF.transform.position, SharkPos);
                if (closestDeadFish == null || distance < currDist)
                {
                    closestDeadFish = DF;
                    distance = currDist;
                }
